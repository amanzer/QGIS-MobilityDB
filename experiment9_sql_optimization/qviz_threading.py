...
def create_matrix(result_queue, begin_frame, end_frame, TIME_DELTA_SIZE, PERCENTAGE_OF_OBJECTS, x_min, y_min, x_max, y_max, timestamps, total_frames, GRANULARITY, db):
    ...
    # now_db = time.time()
    rows = db.get_subset_of_tpoints(p_start, p_end, x_min, y_min, x_max, y_max, GRANULARITY, timestamps[0])    
...
class Time_deltas_handler:
    ...
    def __init__(self, qviz):
        ...
        self.arguments = [0, 0, PERCENTAGE_OF_OBJECTS, x_min, y_min, x_max, y_max]
        self.arguments = [str(arg) for arg in self.arguments]
        self.arguments += ["", str(self.total_frames), GRANULARITY.value["name"], MATRIX_DIRECTORY_PATH, DATABASE_NAME, TPOINT_TABLE_NAME, TPOINT_ID_COLUMN_NAME, TPOINT_COLUMN_NAME]
            
        # PATHS
        self.process_B_path = f"{DIRECTORY_PATH}/QGIS-MobilityDB/experiment9_sql_optimization/optimized_matrix_generation.py"
    ...
    def fetch_next_data(self, time_delta_key):
        ...
        if end_frame  <= self.total_frames and beg_frame >= 0: #Either bound has to be valid 
            ...
            task = Matrix_generation_thread2(f"Data for time delta {time_delta_key} : {self.timestamps_strings[time_delta_key]}","qViz", beg_frame, end_frame,
                                     self.db2, self.qviz.get_canvas_extent(), self.timestamps, self.arguments, self.process_B_path, self.python_path, self.set_matrix2, self.raise_error)
            self.task_manager.addTask(task)        
...
class Matrix_generation_thread2(QgsTask):
    ...
    def run(self):
        ...
        try:
            ...
            thread = threading.Thread(target=create_matrix, args=(result_queue, self.begin_frame, self.end_frame, TIME_DELTA_SIZE, PERCENTAGE_OF_OBJECTS, -180, -90, 180, 90, self.timestamps,len(self.timestamps), 'SECOND',self.db))
            thread.start()
