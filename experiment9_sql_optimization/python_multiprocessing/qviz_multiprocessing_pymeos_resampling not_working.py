...
def create_matrix(result_queue, begin_frame, end_frame, TIME_DELTA_SIZE, x_min, y_min, x_max, y_max, timestamps, GRANULARITY, ids_list):
    """
    This function is executed in parallel to fetch the data from the MobilityDB database for the given time delta.
    """

    logs = f"begin_frame : {begin_frame}, end_frame : {end_frame}, TIME_DELTA_SIZE : {TIME_DELTA_SIZE}, x_min : {x_min}, y_min : {y_min}, x_max : {x_max}, y_max : {y_max}, timestamps[0] : {timestamps[0]}, timestamps[-1] : {timestamps[-1]}, GRANULARITY : {GRANULARITY}, ids_list : {ids_list}\n"
...
class Time_deltas_handler:
    ...
    def __init__(self, qviz):
        ...
        self.x_min, self.y_min, self.x_max, self.y_max = self.qviz.get_canvas_extent()

        x_min,y_min, x_max, y_max = self.qviz.get_canvas_extent()
        ...
        #                              self.db, self.qviz.get_canvas_extent(), self.timestamps_strings[0], self.arguments, self.process_B_path, self.python_path, self.set_matrix, self.raise_error)

        task_matrix_gen = Matrix_generation_thread2(f"Data for time delta {time_delta_key} : {self.timestamps_strings[time_delta_key]}","qViz", beg_frame, end_frame,
                                     self.db.get_ids(), self.x_min, self.y_min, self.x_max, self.y_max, self.timestamps, self.set_matrix2, self.raise_error)
    ...
    def initiate_animation(self):
        ...
        #                              self.db, self.qviz.get_canvas_extent(), self.timestamps_strings[beg_frame],self.arguments, self.process_B_path, self.python_path, self.set_matrix, self.raise_error)

        task = Matrix_generation_thread2(f"Data for time delta {time_delta_key} : {self.timestamps_strings[time_delta_key]}","qViz", beg_frame, end_frame,
                                     self.db.get_ids(), self.x_min, self.y_min, self.x_max, self.y_max, self.timestamps, self.set_matrix2, self.raise_error)
        ...
        if frame_number % TIME_DELTA_SIZE == 0:
            if self.direction == 1:
                ...
                if self.current_time_delta_end + 1  != self.total_frames:
                    ...
                    if self.task_manager.countActiveTasks() != 0:
                        self.log("Data for next batch is being fetched")
                        self.qviz.pause()
...
class Matrix_generation_thread2(QgsTask):
    """
    Creates a thread that fetches data from the MobilityDB database 
    Parameters include : the time delta, STBOX paramters, Time range...
    ...
    This allows to keep the UI responsive while the data is being fetched.
    """
    def __init__(self, description,project_title, beg_frame, end_frame, ids_list, x_min, y_min, x_max, y_max, timestamps, finished_fnc, failed_fnc):
        super(Matrix_generation_thread2, self).__init__(description, QgsTask.CanCancel)
    ...
    def run(self):
        ...
        try:
            ...
            self.log(f"arguments : begin_frame : {self.begin_frame}, end_frame : {self.end_frame}, TIME_DELTA_SIZE : {TIME_DELTA_SIZE}, PERCENTAGE_OF_OBJECTS : {PERCENTAGE_OF_OBJECTS}, self.x_min, self.y_min, self.x_max, self.y_max, len(self.timestamps), GRANULARITY.name,{len(self.ids_list)}")
            process = multiprocessing.Process(target=create_matrix, args=(result_queue, self.begin_frame, self.end_frame, TIME_DELTA_SIZE, PERCENTAGE_OF_OBJECTS,self.x_min, self.y_min, self.x_max, self.y_max, self.timestamps, len(self.timestamps), GRANULARITY.name, self.ids_list))
            process.start()
...
class Matrix_generation_thread(QgsTask):
    ...
    This allows to keep the UI responsive while the data is being fetched.
    """
    def __init__(self, description,project_title, beg_frame, end_frame, db, extent, timestamps, arguments, process_b_path, python_path, finished_fnc, failed_fnc):
        ...
        self.db = db
        self.extent = extent
        self.begin_timestamp = timestamps
        self.finished_fnc = finished_fnc
        self.failed_fnc = failed_fnc
        self.arguments = arguments
        self.process_B_path = process_b_path
    ...
    def run(self):
        ...
        try:
            ...
            self.arguments[0] = str(self.begin_frame)
            self.arguments[1] = str(self.end_frame)
            self.arguments[7] = self.begin_timestamp
            
            # self.log(f"{self.arguments}")
            command = [self.python_path, self.process_B_path, *self.arguments]
...
class QVIZ:
    ...
    def __init__(self):    
        ...
        self.log("Interaction with canvas - Animation stopped")
