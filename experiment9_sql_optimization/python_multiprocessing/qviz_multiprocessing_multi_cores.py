...
class Time_deltas_handler:
    ...
    def create_matrix(self, result_queue, begin_frame, end_frame, TIME_DELTA_SIZE, extent, timestamps, connection_params, table_name, id_column_name, tpoint_column_name, GRANULARITY, ids_str, total_ids):
        ...
        # chunk_size = len(total_ids) // num_workers

        pool = multiprocessing.Pool(num_workers)

        try:
            # (ids, begin_frame, end_frame, TIME_DELTA_SIZE, start_date, empty_point_wkt, connection_params, GRANULARITY, tpoint_column_name, table_name, id_column_name, extent, timestamps) for ids in ids_sub_list]
            # worker_args = [(ids, begin_frame, end_frame, TIME_DELTA_SIZE, start_date, empty_point_wkt, connection_params, GRANULARITY.value["steps"], GRANULARITY.value["name"], tpoint_column_name, table_name, id_column_name, extent, timestamps) for ids in ids_sub_list]
            ...
            # logs += f"results{results}\n"
            for i, (chunk_matrix, worker_logs) in enumerate(results):
                # logs += f"Worker {i} : matrix shape :{chunk_matrix.shape} logs : {worker_logs}\n"
