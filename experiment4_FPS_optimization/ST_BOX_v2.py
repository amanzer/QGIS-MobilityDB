...
class Data_in_memory:
    ...
    def __init__(self, xmin, ymin, xmax, ymax):
        ...
        task = ParallelTask(f"Next batch is being requested: {0}", 0, self.timestamps_strings[0],
                                     "qViz",self.db,self.mmsi_list, self.timestamps[0], self.timestamps[TIME_DELTA-1], self.xmin, self.ymin, self.xmax, self.ymax , self.timestamps, self.finish, self.raise_error)
    ...
    def second_batch(self):
        ...
        """
        self.fetchMobilityDB(TIME_DELTA, self.timestamps_strings[TIME_DELTA], self.mmsi_list, self.timestamps[TIME_DELTA], self.timestamps[2*TIME_DELTA], self.xmin, self.ymin, self.xmax, self.ymax)
    ...
    def fetch_batch(self, start_frame, end_frame, xmin, ymin, xmax, ymax):
        ...
        if delta_key not in self.buffer and start_frame <= self.steps - TIME_DELTA:
            self.fetchMobilityDB(start_frame, delta_key, self.mmsi_list, self.timestamps[start_frame], self.timestamps[end_frame], xmin, ymin, xmax, ymax)
...
class ParallelTask(QgsTask):
    ...
    def run(self):
        ...
        """
        try:
            features = self.db.getTrajectories(self.mmsi_list, self.pstart, self.pend, self.xmin, self.ymin, self.xmax, self.ymax)
...
class qviz:
    ...
    def __init__(self):
        ...
        #iface.mapCanvas().extentsChanged.connect(self.on_extent_change)
        
 
    # def on_extent_change(self):
    ...
    #         # Going back in time
    #         curr_frame = self.current_time_delta
    ...
    #         self.data.fetch_batch(start, end, xmin, ymin, xmax, ymax)   
    ...
    def on_new_frame(self):
        ...
        # check is self.xmin xmas etc have changed
        new_xmin = iface.mapCanvas().extent().xMinimum()
        new_ymin = iface.mapCanvas().extent().yMinimum()
        new_xmax = iface.mapCanvas().extent().xMaximum()
        new_ymax = iface.mapCanvas().extent().yMaximum()
        if new_xmin != self.xmin or new_ymin != self.ymin or new_xmax != self.xmax or new_ymax != self.ymax:
            self.data.delete_time_delta(self.current_time_delta)
            self.xmin = new_xmin
            self.ymin = new_ymin
            self.xmax = new_xmax
            self.ymax = new_ymax
            
            start =self.current_time_delta 
            end = self.current_time_delta  + TIME_DELTA
            self.data.update_keys_to_keep(self.current_time_delta , self.direction)
            self.data.flush_buffer()
            self.data.fetch_batch(start, end, self.xmin, self.ymin, self.xmax, self.ymax)   
