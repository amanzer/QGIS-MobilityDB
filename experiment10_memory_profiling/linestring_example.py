...
PERCENTAGE_OF_OBJECTS = 0.18 # To not overload the memory, we only take a percentage of the ships in the database
            ...
            for i in range(len(rows)):
                if rows[i][2] is not None:
                    try:
                        ...
                        start_point = traj_resampled.values()[0]
                        values = np.array([ LineString([start_point, point]).wkt   for point in traj_resampled.values()])
...
class QVIZ:
    ...
    def create_vlayer(self):
        ...
        """
        self.vlayer = QgsVectorLayer("LineString", "MobilityBD Data", "memory")
