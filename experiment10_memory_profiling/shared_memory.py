...
class Time_deltas_handler:
    ...
    def create_matrix(self, result_queue, shm_name, matrix_shape, begin_frame, end_frame, TIME_DELTA_SIZE, extent, timestamps, connection_params, table_name, id_column_name, tpoint_column_name, GRANULARITY, ids_str):
        """
        ...
        try: 
            ...
            matrix = np.ndarray(matrix_shape, dtype=object, buffer=shm.buf)
...
class Matrix_generation_thread(QgsTask):
    ...
    def run(self):
        ...
        try:
            ...
            matrix_shape = (5821, TIME_DELTA_SIZE)  # Replace EXPECTED_NUM_ROWS with your expected number of rows
            shm = SharedMemory(create=True, size=np.prod(matrix_shape) * np.dtype(object).itemsize)
            result_matrix = np.ndarray(matrix_shape, dtype=object, buffer=shm.buf)
            empty_point_wkt = Point().wkt  # "POINT EMPTY"
            result_matrix.fill(empty_point_wkt)  # Initialize with empty points
            ...
            process = multiprocessing.Process(target=self.create_matrix, args=(result_queue, shm.name, matrix_shape, self.begin_frame, self.end_frame, TIME_DELTA_SIZE, self.extent, self.timestamps, connection_params, TPOINT_TABLE_NAME, TPOINT_ID_COLUMN_NAME, TPOINT_COLUMN_NAME, GRANULARITY, self.objects_id_str))
            process.start()
    ...
    def log(self, msg):
        QgsMessageLog.logMessage(msg, self.project_title, level=Qgis.Info)
