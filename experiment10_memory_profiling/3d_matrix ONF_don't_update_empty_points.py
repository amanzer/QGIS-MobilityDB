...
class Time_deltas_handler:
    ...
    def update_vlayer_features(self):
        ...
        try:
            ...
            if frame_number == 0:
                previous_positions = self.matrices[0][:, TIME_DELTA_SIZE-1]
                new_positions = self.matrices[1][:, 0]
            else:
                previous_positions = self.matrices[1][:, frame_index-1]
                new_positions = self.matrices[1][:, frame_index]
            
            previous_empty = np.vectorize(self.is_empty_point)(previous_positions)
            new_empty = np.vectorize(self.is_empty_point)(new_positions)

            valid_indices = np.where(~previous_empty & ~new_empty)[0]
            new_geometries = {index: QgsGeometry().fromWkt(new_positions[index]) for index in valid_indices}
